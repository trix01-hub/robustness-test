...
def main(params):
    ...
    all_row_id = []
    all_models = []
    modelArray = params['exp_ids'].split()
    evaluate_folder = params['evaluate_folder']
    if not os.path.exists(evaluate_folder):
        os.mkdir(evaluate_folder)
    for model in modelArray:
        params['exp_id'] = model

        for model_row_id in range(20):
#            model_row_id = 19
            ...
            if params['sarsa_enable']:
                ...
                for i in range(num_episodes):
                    ...
                    ## original_stdv = None with stdv dependent on state
                    try:
                        ep_length, ep_reward, ep_avg_adiv, actions, action_means, states, kl_certificates, steps, velocities = p.run_test(compute_bounds=params['compute_kl_cert'], use_full_backward=params['use_full_backward'], original_stdev=original_stdev)
                    except:
                        ep_length, ep_reward, ep_avg_adiv, actions, action_means, states, kl_certificates, steps, velocities = p.run_test(compute_bounds=params['compute_kl_cert'], use_full_backward=params['use_full_backward'], original_stdev=None)
                    ...
                    all_velocities.extend([sum(velocities) / len(velocities)])
                    all_episodes.extend(episodes)
                    all_row_id.extend(row_ids)
    ...
    excel_file_path = f"{evaluate_folder}/{params['excel_name']}.xlsx"
...
def get_parser():
    ...
    parser.add_argument('--exp-ids', type=str, help='experiement ids for testing', default='0')
    parser.add_argument('--evaluate-folder', type=str, default='default_evaluate_folder')
    ...
    # Other configs
    parser.add_argument('--excel-name', type=str, help='save results to an excel given name.', default='')
